...
app.controller('mainCtrl', function($scope, itunesService){
  ...
  //box on index.html, head over there and check if that input box is tied to any specific model we could 
  //use.
  ...
  $scope.getSongData = function(){
     itunesService.getArtist($scope.artist).then(function(songs){
      console.log(songs)
      var correctlyFormatedSongs = mapItunesFormatIntoWhatWeWant(songs)
      $scope.songData = correctlyFormatedSongs
     })
  }

  function mapItunesFormatIntoWhatWeWant(itunesSongs){
    return itunesSongs.map(function(itunesSong){
      return{
        Artist: itunesSong.artistName,
        Collection: itunesSong.collectionName,
        AlbumArt: itunesSong.artworkUrl100,
        Type: itunesSong.wrapperType,
  ...
  //If everything worked you should see a huge array of objects inside your console. That's great! But 
  //unfortunately that's not what ng-grid is expecting. What you need to do now
  //is sort the data you got back to be an object in the following format.
    /*
      ...
      Type: "song"
  */
  // the iTunes API is going to give you a lot more details than ng-grid wants. Create a new array and
  // then loop through the iTunes data pushing into your new array objects that look like the above data.
  
  //Once you have that final data array, you simply need to put it on the 
  //scope (or more specifically on 
  //the scope as songData). Once you do this ($scope.songData = myFinalArray) 
  //then ng-grid will see that 
